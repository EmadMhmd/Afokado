...
applyController.fetchApplications = async (req, res, next) => {
    ...
    try {
        const applications = await Apply.find({trainee: user._id,deleted: 0}).populate("internshipId");
        return res.send({
            applications
        });
    } catch (e) {
...
applyController.fetchApplicationRequests = async (req, res, next) => {
    ...
    const {id}=req.params
    try {
        const applications = await Apply.find({ lawyer: user._id , internshipId:id  , deleted: 0, confirmed: 1, status: { $in: ['pending', 'accept'] } }).populate("trainee internshipId");
        return res.send({
            applications
        });
    } catch (e) {
...
applyController.confiremApplication = async (req, res, next) => {
    try {
        ...
        return res.send({
            message: 'You was confirmed your application successfully',
        })
    } catch (e) {
        return res.status(401).send({
...
applyController.rejectApplication = async (req, res, next) => {
    try {
        ...
        return res.send({
            message: 'The application rejected successfully',
        })
    } catch (e) {
        return res.status(401).send({
            error: 'Please try again to confirm your application !!'
...
applyController.acceptApplication = async (req, res, next) => {
    try {
        ...
        return res.send({
            message: 'The application accepted  successfully',
        })
    } catch (e) {
        return res.status(401).send({
            error: 'Please try again to confirm your application !!'
